...
import Player, { ServerPlayer, TrainerName, UserLocation } from './classes/Player';

import Pokemon, { ServerPokemon } from './classes/Pokemon';
...
import AchievementCountContext, { DEFAULT_ACHIEVEMENT_COUNTS } from './contexts/AchievementCountContext';
import AchievementListContext, { DEFAULT_ACHIEVEMENT_LIST } from './contexts/AchievementListContext';
...
import { AchievementList, AchievementTypeCount, CoveyAppState } from './CoveyTypes';
...
function defaultAppState(): CoveyAppState {
  return {
    myPlayerID: '',
    myTrainerName: TrainerName.lucas,
...
function appStateReducer(state: CoveyAppState, update: CoveyAppUpdate): CoveyAppState {
  const nextState = {
    ...
    myTrainerName: state.myTrainerName,
    userName: state.userName,
    socket: state.socket,
    emitMovement: state.emitMovement,
    emitCatch: state.emitCatch,
  ...
  switch (update.action) {
    case 'doConnect':
      ...
      nextState.myTrainerName = update.data.myTrainerName;
      ...
      nextState.emitCatch = update.data.emitCatch;
...
function App(props: { setOnDisconnect: Dispatch<SetStateAction<Callback | undefined>> }) {
  ...
  const [nearbyPlayers, setNearbyPlayers] = useState<Player[]>([]);
	...
	// Pokemon
	const [pokemonInTown, setPokemonInTown] = useState<Pokemon[]>([]);
  ...
  const [achievementList, setAchievementList] = useState<AchievementList>(DEFAULT_ACHIEVEMENT_LIST);

  const [achievementCounts, setAchievementCounts] = useState<AchievementTypeCount>(DEFAULT_ACHIEVEMENT_COUNTS);

  const setupGameController = useCallback(
    async (initData: TownJoinResponse) => {
      const gamePlayerID = initData.coveyUserID;
      const gameTrainerName = initData.trainerName;
      ...
      let localPlayers = initData.currentPlayers.map(sp => Player.fromServerPlayer(sp));
			let localPokemon = initData.currentPokemon.map(sp => Pokemon.fromServerPokemon(sp));
      ...
      const emitCatch = (pokemon: ServerPokemon, userID: string) => {
        socket.emit('pokemonCaught', pokemon, userID);
      };
      ...
      socket.on('newPlayer', (player: ServerPlayer) => {
        ...
        recalculateNearbyPlayers();
			...
			socket.on('newPokemon', (pokemon: ServerPokemon) => {
        localPokemon = localPokemon.concat(Pokemon.fromServerPokemon(pokemon));
        setPokemonInTown(localPokemon);
      });
      ...
      socket.on('removedPokemon', (removedPokemon: ServerPokemon) => {
        localPokemon = localPokemon.filter(p => p._id !== removedPokemon._id);
        setPokemonInTown(localPokemon);
      });
      ...
      socket.on('playerPokemonUpdated', (updatedPlayer: ServerPlayer) => {
        if (updatedPlayer._id !== gamePlayerID) {
          playerMovementCallbacks.forEach(cb => cb(updatedPlayer));
        }
        const myPlayer = localPlayers.find(p => p.id === updatedPlayer._id);
        if (myPlayer) {
          const combinedList: Pokemon[] = [];
          // ServerPlayer input does not have sprites, use the old list to maintain sprites
          updatedPlayer.pokemon.forEach(newP => {
      ...
      socket.on('achievementsUpdated', (updatedList: AchievementList) => {
        setAchievementList(updatedList);
      });
      socket.on('achievementCountsUpdated', (updatedCounts: AchievementTypeCount) => {
        setAchievementCounts(updatedCounts);
      })
      dispatchAppUpdate({
        action: 'doConnect',
        data: {
          ...
          <PlayerMovementContext.Provider value={playerMovementCallbacks}>
            <PlayersInTownContext.Provider value={playersInTown}>
              <NearbyPlayersContext.Provider value={nearbyPlayers}>
                <PokemonInTownContext.Provider value={pokemonInTown}>
                  <ConversationAreasContext.Provider value={conversationAreas}>
                    <AchievementListContext.Provider value={achievementList}>
                      <AchievementCountContext.Provider value={achievementCounts}>
                        {page}
                      </AchievementCountContext.Provider>
                    </AchievementListContext.Provider>
                  </ConversationAreasContext.Provider>
                </PokemonInTownContext.Provider>
