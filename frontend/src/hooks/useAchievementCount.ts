...
import AchievementCountContext from '../contexts/AchievementCountContext';
 ...
 * 
 */
export default function useConversationAreas(): AchievementTypeCount {
  const ctx = useContext(AchievementCountContext);
  assert(ctx, 'Achievement counts context should be defined.');
  return ctx;
}
