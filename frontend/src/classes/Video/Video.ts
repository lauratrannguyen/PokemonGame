...
import { TrainerName } from '../Player';
...
export default class Video {
  ...
  private async setup(trainerName: TrainerName): Promise<TownJoinResponse> {
  ...
  public static async setup(username: string, coveyTownID: string, trainerName: TrainerName): Promise<TownJoinResponse> {
    let result = null;
    ...
    try {
      result = await Video.video.setup(trainerName);
      if (!result) {
        Video.video = null;
      }
    } catch (err) {
