...
import { ChatMessage, CoveyTownList, TrainerName, UserLocation } from '../CoveyTypes';
import CoveyTownListener from '../types/CoveyTownListener';
import CoveyTownsStore from '../lib/CoveyTownsStore';
import { ConversationAreaCreateRequest, ServerConversationArea, ServerPokemon } from '../client/TownsServiceClient';
import Pokemon from '../types/Pokemon';
import { AchievementList, AchievementTypeCount } from '../lib/TownAchievementManager';
...
export interface TownJoinRequest {
  ...
  coveyTownID: string;
  /** the name for the trainer avatar */
...
export interface TownJoinResponse {
  ...
  /** List of pokemon currently in this town * */
...
export async function townJoinHandler(requestData: TownJoinRequest): Promise<ResponseEnvelope<TownJoinResponse>> {
  ...
  const newPlayer = new Player(requestData.userName, requestData.trainerName);
  ...
  return {
    ...
    response: {
      ...
      trainerName: newPlayer.trainerName,
      ...
      currentPokemon: coveyTownController.pokemon,
...
function townSocketAdapter(socket: Socket): CoveyTownListener {
  return {
    ...
    onPokemonSpawned(newPokemon: Pokemon) {
      socket.emit('newPokemon', newPokemon);
    },
    onPokemonRemoved(removedPokemon: Pokemon) {
      socket.emit('removedPokemon', removedPokemon);
    },
    onPlayerUpdatePokemon(player: Player) {
      socket.emit('playerPokemonUpdated', player);
    },
  };
}
...
export function townSubscriptionHandler(socket: Socket): void {
  ...
  townController.townAchievementManager.setSocketEmitters((achievementList: AchievementList) => {
    socket.emit('achievementsUpdated', achievementList);
  }, (achievementCounts: AchievementTypeCount) => {
    socket.emit('achievementCountsUpdated', achievementCounts);
  });
  ...
  socket.on('pokemonCaught', (caughtPokemon: ServerPokemon, userID: string) => {
    townController.onPokemonCaught(caughtPokemon, userID);
  });
}
