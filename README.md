## Adding Pokemon
1. Add a name to PokemonName (services/townService/src/CoveyTypes.ts)
2. Add a spawn rate to POKEMON_SPAWN_RATES (services/townService/src/CoveyTypes.ts)
3. if you would like it to be a default (not unlocked by achievement) pokemon, add it to DEFAULT_POKEMON (services/townService/src/CoveyTypes.ts)
4. Add front and back images to (frontend/public/assets/pokemon & backPokemon)
5. Add 6 images to animate the pokemon to (frontend/public/assets/pokemonGif)

## Adding Achievements
1. add a key to AchievementKey (services/townService/src/CoveyTypes.ts)
2. add an entry to TownAchievementManger => defaultAchievementList() (TownAchievementManager.ts)
